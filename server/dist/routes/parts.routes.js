...
const showParts = async (req, res, next) => {
    try {
        const parts = await (0, database_js_1.getParts)();
        res.status(200).json({
            success: true,
            parts
        });
...
const regPart = async (req, res, next) => {
    try {
        const { oem_code, part_name, category, unit_cost, selling_price, stock_quantity, reorder_threshold, supplier_id } = req.body;
        ...
        await (0, database_js_1.createPart)(oem_code, part_name, categoryVal, unit_cost, selling_price, stockVal, thresholdVal, supplier_id);
        res.status(200).json({
            success: true,
...
const removePart = async (req, res) => {
    try {
        const { id } = req.params;
        await (0, database_js_1.deletePart)(Number(id));
        res.status(200).json({
            success: true,
...
const usePartQuantity = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const result = await (0, database_js_1.usePart)(Number(id), quantity);
